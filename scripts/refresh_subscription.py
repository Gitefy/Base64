...
def delay_one(c):
    ...
    if len(successful_rounds) < REQUIRED_SUCCESS_ROUNDS:
        return None
...
def main():
    ...
    for u in old_lines:
        ...
        if x:
            parsed.append(x)
            rec=health_nodes.setdefault(x["key"],{"ever_passed":True,"fail_streak":0})
            rec["ever_passed"]=True
            rec.setdefault("fail_streak",0)
    for u in new_lines:
        x=parse_uri(u,"qzz")
        if x: parsed.append(x)
    ...
    # deleted after one transient bad run. It gets one grace cycle; a second
    # consecutive failed run removes it unless it recovers.
    for c in candidates:
        rec=health_nodes.setdefault(c["key"],{"ever_passed":False,"fail_streak":0})
        if c["key"] in strict_keys:
            rec["ever_passed"]=True
            rec["fail_streak"]=0
            rec["last_pass_utc"]=now_utc
            rec.pop("last_fail_utc",None)
    ...
    selected_endpoints={(str(c.get("server","")).lower(),int(c.get("port",0) or 0)) for c in selected}
    grace_selected=[]
    for c in grace_candidates:
        endpoint=(str(c.get("server","")).lower(),int(c.get("port",0) or 0))
        if c["key"] in selected_keys or endpoint in selected_endpoints:
            continue
        c["grace_retained"]=True
        grace_selected.append(c)
        selected_keys.add(c["key"])
        selected_endpoints.add(endpoint)

    final_selected=selected+grace_selected
    if not final_selected:
        raise RuntimeError("URL test found zero usable or grace-retained nodes")
    ...
    latency_stats={
        ...
        "median":sorted(latency_values)[len(latency_values)//2] if latency_values else None
    }
    ...
    stats={
        ...
        "required_success_rounds":REQUIRED_SUCCESS_ROUNDS,"grace_fail_runs":GRACE_FAIL_RUNS,"max_delay_ms":MAX_DELAY_MS,
        "endpoint_duplicates_removed":endpoint_duplicates_removed,
        "usable_by_country":counts,
        "strict_selected_by_country":{k:sum(1 for x in selected if x["country"]==k) for k in ("US","SG","JP")},
        "grace_retained_by_country":{k:sum(1 for x in grace_selected if x["country"]==k) for k in ("US","SG","JP")},
        "selected_by_country":{k:sum(1 for x in final_selected if x["country"]==k) for k in ("US","SG","JP")},
        "quota_if_capped":q,
        "selected_latency_ms":latency_stats,
